...
const authResolvers = {
    Query: {
        aboutMe: compose(...authGuard)(async (parent, args, ctx, info) => {
            const user = await User.findOne({ where: { firebase_uuid: ctx.user.firebase_uuid, email: ctx.user.email }, raw: true })
    ...
    Mutation: {
        doLogin: async (parent, args, ctx, info) => {
            ...
            const user = await User.findOne({ where: { firebase_uuid: args.input.firebaseUUID, email: args.input.email } })
            ...
            const token = jwt.sign({ id: user.firebase_uuid, sub: user.email }, process.env.JWT_SECRET, {
                expiresIn: expiresIn
            });
