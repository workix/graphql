export default class CompanyDTO {
  [key: string]: any;
    constructor(company){
        this.id = company.id
        this.createdAt = company.created_at
        this.updatedAt = company.updated_at
        this.uuid = company.uuid
        this.mobilePhone = company.mobile_phone
        this.phone = company.mobile_phone ? String(company.mobile_phone) : (company.phone || '')
        this.city = company.city
        this.state = company.state
        this.location = (company.city && company.state) ? `${company.city}, ${company.state}` : (company.city || company.location || '')
        this.neighborhood = company.neighborhood
        this.number = company.number
        this.street = company.street
        this.zipCode = company.zip_code
        this.name = company.name
        this.cnpj = company.cnpj ? String(company.cnpj) : ''
        this.description = company.description
        this.logo = company.logo
        this.segment = company.segment
        this.industry = company.segment || company.industry || 'Tecnologia'
        this.userId = company.user_id
        this.email = company.email || ''
    }
}
