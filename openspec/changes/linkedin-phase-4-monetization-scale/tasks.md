...
- [x] 2.2 Criar módulo GraphQL `src/modules/premium` com schema, DTOs, repositório e resolvers para planos e assinaturas; criar composable `requirePlanResolver` em `src/composable_resolvers`
- [x] 2.3 Estender `src/modules/messaging` (`sendMessage`) para permitir InMail a não-conexões com plano ativo, decrementando créditos mensais
- [x] 2.4 Criar/estender suítes de testes unitários TDD (`tests/unit/modules/premium.spec.ts` e extensão de `tests/unit/modules/messaging.spec.ts`) com 100% de cobertura
...
- [x] 4.2 Criar módulo GraphQL `src/modules/social_selling` com schema, DTOs, repositório e resolver de cálculo/consulta do score
- [x] 4.3 Criar suíte de testes unitários TDD em `tests/unit/modules/social_selling.spec.ts` com 100% de cobertura
