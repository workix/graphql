...
Os componentes Vue essenciais (`JobCard.vue`, `PostCard.vue`, `LoadingOverlay.vue`, `Pagination.vue`, `TheFooter.vue`) SHALL possuir testes unitários verificando a renderização correta de props, emissão de eventos e reatividade com `@vue/test-utils`.
