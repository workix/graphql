# Fase 4 — Monetização e Escala (Especificação Principal)
...
- **WHEN** o usuário executar a mutation `requestIdentityVerification(userId: ID!, method: "WORK_EMAIL")` e, em seguida, `confirmIdentityVerification(token: String!)` com o token JWT de confirmação recebido
