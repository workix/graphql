...
- **Comportamento atual**: Cobertura global fica em ~89% statements / ~84% branches / ~91% lines / ~83% functions (medido durante a Fase 4, após os módulos `hashtags` e `premium` - todos com 100% de cobertura individual). O módulo `messaging` foi identificado com gap de cobertura pré-existente (fallback `db.Sequelize.Op?.or || '$or'` e `ctx.pubsub || pubsub`, nunca exercitados na branch alternativa) ao ser estendido para o fluxo de InMail na Fase 4.
